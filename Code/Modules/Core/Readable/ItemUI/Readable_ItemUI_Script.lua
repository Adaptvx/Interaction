...
function NS.ItemUI.Script:Load()
	...
	do
		...
		function NS.ItemUI.Script:Update()
			...
			local item_parchmentGradientTexture = addon.Theme.IsDarkTheme and NS.Variables.READABLE_UI_PATH .. "Parchment\\ParchmentContentFade-Dark.png" or NS.Variables.READABLE_UI_PATH .. "Parchment\\ParchmentContentFade-Light.png"

			local item_stoneTexture = NS.Variables.READABLE_UI_PATH .. "Slate\\Slate.png"
			local item_stoneGradientTexture = NS.Variables.READABLE_UI_PATH .. "Slate\\SlateContentFade.png"

			local book_coverTexture
			local book_contentTexture
			if type == "Parchment" or type == nil then
				book_coverTexture = NS.Variables.READABLE_UI_PATH .. "Book\\BookCover.png"
				book_contentTexture = NS.Variables.READABLE_UI_PATH .. "Book\\Book.png"
			else
				book_coverTexture = NS.Variables.READABLE_UI_PATH .. "Book\\BookCoverLarge.png"
				book_contentTexture = NS.Variables.READABLE_UI_PATH .. "Book\\BookLarge.png"
			end
			...
			do -- Text
				...
				do -- Content
					if ReadableUI_ItemUI:IsVisible() then
						ReadableUI_ItemUI.ScrollFrame.Text.Text:SetText(NS.ItemUI.Variables.Content[NS.ItemUI.Variables.CurrentPage])
					end
					...
					if ReadableUI_BookUI:IsVisible() then
						...
						if currentPage <= 1 then
							...
							ReadableUI_BookUI.Content.Left.Text.Text:SetText(NS.ItemUI.Variables.Content[tonumber(NS.ItemUI.Variables.CurrentPage) - 1] or "")
							ReadableUI_BookUI.Content.Right.Text.Text:SetText(NS.ItemUI.Variables.Content[tonumber(NS.ItemUI.Variables.CurrentPage)] or "")
			...
			do -- Text color
				...
				ReadableUI_ItemUI.ScrollFrame.Text.Text:SetTextColor(textColor.r, textColor.g, textColor.b, textColor.a)
			end
				...
				do -- Book
					if ReadableUI_BookUI:IsVisible() then
						if (ReadableUI_BookUI.FrontPage:IsVisible()) or ((NS.ItemUI.Variables.CurrentPage - 1) == 0) then
							if ReadableUI_BookUI.FrontPage:IsVisible() then
								addon.SoundEffects:PlaySound(831)
							else
								addon.SoundEffects:PlaySound(831)
							end
	...
	do
		local function Settings_ContentSize()
			...
			addon.API.Util:SetFontSize(Frame.ReadableUIFrame.ItemFrame.ScrollFrame.MeasurementText.Text, item_textSize)
			addon.API.Util:SetFontSize(Frame.ReadableUIFrame.ItemFrame.ScrollFrame.Text.Text, item_textSize)
			addon.API.Util:SetFontSize(Frame.ReadableUIFrame.BookFrame.Content.Left.MeasurementText.Text, book_textSize)
			addon.API.Util:SetFontSize(Frame.ReadableUIFrame.BookFrame.Content.Left.Text.Text, book_textSize)
			addon.API.Util:SetFontSize(Frame.ReadableUIFrame.BookFrame.Content.Right.MeasurementText.Text, book_textSize)
			addon.API.Util:SetFontSize(Frame.ReadableUIFrame.BookFrame.Content.Right.Text.Text, book_textSize)
