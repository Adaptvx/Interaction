	...
	do
		do -- Data
			...
			function Callback:ReadLine(line, quotation)
				...
				C_Timer.After(0, function()
					addon.TextToSpeech.Script:SpeakText(voice, line, rate, volume)
				end)
			end
		...
		do -- Frame
			...
			function Frame:UpdateState()
				...
				if NS.Variables.IsPlaying then
					Frame.Content.PlaybackButton.ImageTexture:SetTexture(NS.Variables.AUDIOBOOKUI_PATH .. "Pause.png")

					if Frame.TextPreviewFrame.hidden then
						Frame.TextPreviewFrame:ShowWithAnimation()
					end
				else
					Frame.Content.PlaybackButton.ImageTexture:SetTexture(NS.Variables.AUDIOBOOKUI_PATH .. "Play.png")
