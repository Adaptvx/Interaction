...
local function IsUnitPlayer(unitToken)
	return UnitExists(unitToken) and UnitIsUnit(unitToken, "player")
end

local function IsInteractionTargetPlayer()
	return IsUnitPlayer("npc") or IsUnitPlayer("questnpc")
end

local function IsMouseOverInteractionTarget()
...
function NS.Script:Load()
	...
	do
		...
		do -- Style
			...
			function Frame:Style_MatchScrollCriteria()
				...
				local targetIsSelf = IsInteractionTargetPlayer()
				local targetIsItem = (addon.API.Util:FindItemInInventory(addon.API.Util:GetValueOrFallback(UnitName("npc"), "Empty Result")) or addon.API.Util:FindItemInInventory(addon.API.Util:GetValueOrFallback(UnitName("questnpc"), "Empty Result")))
	...
	do
		do -- Get
			...
			local function SplitText(string, splitParagraphs)
				if not string then
					return
				end
				if addon.API.Util:IsSecretValue(string) then
					return { string }
				end
				...
				for segment in SplitString(string, separatorPattern) do
					if segment then
						local trimmedSegment = segment:gsub("^%s+", ""):gsub("%s+$", "")
						if addon.API.Util:HasVisibleText(trimmedSegment) then
							lines[lineIndex] = trimmedSegment
			...
			local function RemoveAngledBrackets(str)
				if addon.API.Util:IsSecretValue(str) then
					return str
				end
			...
			function Callback:GetNPCInfo()
				local npcInfo = {
					["name"] = addon.API.Util:GetFirstValue(UnitName("npc"), UnitName("questnpc")),
					["guid"] = addon.API.Util:GetFirstValue(UnitGUID("npc"), UnitGUID("questnpc"))
				}
			...
			function Callback:GetContent(frameType)
				...
				local splitParagraphs = addon.Database.DB_GLOBAL.profile.INT_DIALOG_SPLIT_PARAGRAPHS
				...
				local text_split =
					frameType == "quest-reward" and SplitText(TEXT_REWARD, splitParagraphs) or
					frameType == "quest-progress" and SplitText(TEXT_PROGRESS, splitParagraphs) or
					frameType == "gossip" and SplitText(TEXT_GOSSIP, splitParagraphs) or
					frameType == "quest-greeting" and SplitText(TEXT_GREETING, splitParagraphs) or
					frameType == "quest-detail" and SplitText(TEXT_QUEST, splitParagraphs)
				...
				if text_split then
					for i = 1, #text_split do
						...
						if addon.API.Util:IsSecretValue(formatted.text) then
							formatted.skipAnimation = true
						else
							formatted.text = formatted.text:gsub("%.%.%.", "…")
						end
						if addon.API.Util:FindString(formatted.text, "|cFFFF0000") then
							formatted.skipAnimation = true
						end
		...
		do -- Set
			...
			function Callback:SetInfo(frameType)
				...
				NS.Variables.ContentVersion = NS.Variables.ContentVersion + 1
				...
				NS.Variables.info.contentInfo.version = NS.Variables.ContentVersion
			end

			function Callback:SetLineToIndex(index, skipAnimation, preventAutoProgress)
				local info = NS.Variables.info
				local SECRET_TITLE_WIDTH = 190

				local function SetSecretTitleText(prefix, text)
					local displayPrefix = addon.API.Util:GetDisplayValueOrFallback(prefix, "")
					Frame.REF_TITLE:SetWidth(SECRET_TITLE_WIDTH)
					Frame.REF_TITLE_SECRETTITLEFRAME:SetWidth(SECRET_TITLE_WIDTH)
					Frame.REF_TITLE_SECRETTITLEFRAME_ICON:SetText(displayPrefix)
					Frame.REF_TITLE_SECRETTITLEFRAME_TEXT:ClearAllPoints()
					Frame.REF_TITLE_SECRETTITLEFRAME_TEXT:SetPoint("TOP", Frame.REF_TITLE_SECRETTITLEFRAME, addon.API.Util:HasVisibleText(displayPrefix) and 8 or 0, 0)
					Frame.REF_TITLE_SECRETTITLEFRAME_TEXT:SetText(text)
					Frame.REF_TITLE_SECRETTITLEFRAME:Show()
				end

				local function SetTitleFrameText(frame, fontString, prefix, text, useSecretTitleFrame)
					if addon.API.Util:IsSecretValue(text) then
						fontString:SetText("")
						frame:Hide()
						if useSecretTitleFrame then
							SetSecretTitleText(prefix, text)
						end
					else
						local displayText = addon.API.Util:GetDisplayValueOrFallback(text, "")
						if addon.API.Util:HasVisibleText(displayText) then
							local displayPrefix = addon.API.Util:GetDisplayValueOrFallback(prefix, "")
							fontString:SetText(addon.API.Util:HasVisibleText(displayPrefix) and (displayPrefix .. " " .. displayText) or displayText)
							frame:Show()
						else
							fontString:SetText("")
							frame:Hide()
				...
				do -- Set
					...
					do -- Title text
						Frame.REF_TITLE_SECRETTITLEFRAME:Hide()

						if isAlwaysShowQuest and isQuestVisible and isQuestType then
							Frame.REF_TITLE_PREFIXFRAME:Hide()
							SetTitleFrameText(Frame.REF_TITLE_TITLEFRAME, Frame.REF_TITLE_TITLEFRAME_TEXT, info.contextIcon, info.npcInfo.name, true)
						elseif isQuestType then
							SetTitleFrameText(Frame.REF_TITLE_PREFIXFRAME, Frame.REF_TITLE_PREFIXFRAME_TEXT, nil, info.npcInfo.name)
							SetTitleFrameText(Frame.REF_TITLE_TITLEFRAME, Frame.REF_TITLE_TITLEFRAME_TEXT, info.contextIcon, info.title, true)
						else
							Frame.REF_TITLE_PREFIXFRAME:Hide()
							SetTitleFrameText(Frame.REF_TITLE_TITLEFRAME, Frame.REF_TITLE_TITLEFRAME_TEXT, info.contextIcon, info.title, true)
						end
					...
					do -- Content text
						Frame.REF_CONTENT_TEXT:SetText(addon.API.Util:GetDisplayValueOrFallback(info.contentInfo.formatted[index].text, ""))
					end
				end
		...
		do -- State
			...
			function Callback:Init(frameType)
				...
				if info.contentInfo.split and info.contentInfo.split[1] then
	...
	do
		do -- Text
			...
			local function AdjustForEscapeSequences(text, numChars)
				local textLen = strlenutf8(text)
				if numChars >= textLen then
					return numChars
				end

				local currentText = addon.API.Util:GetSubstring(text, 1, numChars)

				local lastTextureStart = currentText:find("|T[^|]*$")
				if lastTextureStart then
					local closePos = text:find("|t", lastTextureStart)
					if closePos then
						local prefix = text:sub(1, closePos + 1)
						return strlenutf8(prefix)
					end
				end

				local lastAtlasStart = currentText:find("|A[^|]*$")
				if lastAtlasStart then
					local closePos = text:find("|a", lastAtlasStart)
					if closePos then
						local prefix = text:sub(1, closePos + 1)
						return strlenutf8(prefix)
					end
			...
			function Frame:Animation_Text_Start(interval, pauseText)
				...
				f:SetScript("OnUpdate", Frame.Animation_Text_OnUpdate)
			end
			...
			function Frame:Animation_Text_OnUpdate(elapsed)
				...
				numCharsToShow = AdjustForEscapeSequences(session.text, numCharsToShow)
		...
		do -- Frame
			local isTransition
			local savedDialogVersion
			local savedDialogIndex
			...
			do -- New dialog
				...
				function Frame:Animation_New_StopEvent()
					return (NS.Variables.info.contentInfo.version ~= savedDialogVersion) or (NS.Variables.Playback_Index ~= savedDialogIndex)
				end
				...
				function Frame:Animation_New()
					...
					if (savedDialogVersion == NS.Variables.info.contentInfo.version) and (savedDialogIndex == NS.Variables.Playback_Index) then
						return
					end
					savedDialogVersion = NS.Variables.info.contentInfo.version
					savedDialogIndex = NS.Variables.Playback_Index
			...
			do -- Show
				...
				function Frame:ShowWithAnimation()
					...
					if not Frame.hidden then
						...
						addon.API.Animation:Fade(Frame.REF_TITLE_SECRETTITLEFRAME, .5, 0, addon.Database.DB_GLOBAL.profile.INT_TITLE_ALPHA, nil, Frame.ShowWithAnimation_StopEvent)
	...
	do
		...
		local function Settings_DialogSplitParagraphs()
			if addon.Interaction.Variables.Active and NS.Variables.info.type then
				Callback:SetInfo(NS.Variables.info.type)
				Frame:UpdateAll()
			end
		end
		Settings_DialogSplitParagraphs()
		...
		CallbackRegistry:Add("SETTINGS_DIALOG_SPLIT_PARAGRAPHS_CHANGED", Settings_DialogSplitParagraphs, 0)
		...
		f:SetScript("OnEvent", function(self, event, ...)
			if event == "GLOBAL_MOUSE_UP" then
				if addon.Interaction.Variables.Active then
					...
					if IsMouseOverInteractionTarget() then
