...
local DEFAULT_CONTENT_TEXT_SIZE = 15
local CONTENT_TEXT_VISUAL_SCALE = 0.8

local function ApplyContentTextSize(fontString)
	if not fontString then
		return
	end

	if not fontString._InteractionBaseContentTextSize then
		local _, size = fontString:GetFont()
		fontString._InteractionBaseContentTextSize = size or DEFAULT_CONTENT_TEXT_SIZE
	end

	addon.API.Util:SetFontSize(fontString, addon.Database.DB_GLOBAL.profile.INT_CONTENT_SIZE * (fontString._InteractionBaseContentTextSize / DEFAULT_CONTENT_TEXT_SIZE) * CONTENT_TEXT_VISUAL_SCALE)
end

local function ApplyContentTextSizeToRewardButtons(buttons)
	if not buttons then
		return
	end
	...
	for i = 1, #buttons do
		...
		if button then
			ApplyContentTextSize(button.Label)

			if button.Image and button.Image.Text and button.Image.Text.Label then
				ApplyContentTextSize(button.Image.Text.Label)
			end

			if button.RefreshLayout then
	...
	do
		...
		function Frame:UpdateWarbandHeader()
			...
			if addon.Theme.IsDarkTheme then
				if isWarbandComplete then
					TEXTURE_Background = NS.Variables.QUEST_PATH .. "HeaderComplete-Dark.png"
				else
					TEXTURE_Background = NS.Variables.QUEST_PATH .. "Header-Dark.png"
				end
			else
				if isWarbandComplete then
					TEXTURE_Background = NS.Variables.QUEST_PATH .. "HeaderComplete-Light.png"
				else
		...
		do -- Buttons
			...
			do
				local function Settings_ContentSize()
					local ScrollContent = Frame.REF_MAIN_SCROLLFRAME_CONTENT
					local Storyline = Frame.REF_HEADER_STORYLINE and Frame.REF_HEADER_STORYLINE.Storyline

					ApplyContentTextSize(Frame.REF_HEADER_TITLE and Frame.REF_HEADER_TITLE.Text)

					if Storyline then
						ApplyContentTextSize(Storyline.Content.Content.LayoutGroup.TextFrame.Text)

						if Storyline.RefreshLayout then
							Storyline:RefreshLayout()
						end
					end

					if ScrollContent then
						ApplyContentTextSize(ScrollContent.Objectives_Header and ScrollContent.Objectives_Header.Text)
						ApplyContentTextSize(ScrollContent.Objectives_Text)
						ApplyContentTextSize(ScrollContent.Rewards_Header and ScrollContent.Rewards_Header.Text)
						ApplyContentTextSize(ScrollContent.Rewards_Experience and ScrollContent.Rewards_Experience.Text)
						ApplyContentTextSize(ScrollContent.Rewards_Currency and ScrollContent.Rewards_Currency.Text)
						ApplyContentTextSize(ScrollContent.Rewards_Honor and ScrollContent.Rewards_Honor.Text)
						ApplyContentTextSize(ScrollContent.Rewards_Choice)
						ApplyContentTextSize(ScrollContent.Rewards_Receive)
						ApplyContentTextSize(ScrollContent.Rewards_Spell)
						ApplyContentTextSize(ScrollContent.Progress_Header and ScrollContent.Progress_Header.Text)
					end

					ApplyContentTextSizeToRewardButtons(Frame:GetButtons_Choice())
					ApplyContentTextSizeToRewardButtons(Frame:GetButtons_Reward())
					ApplyContentTextSizeToRewardButtons(Frame:GetButtons_Required())
					ApplyContentTextSizeToRewardButtons(Frame:GetButtons_Spell())

					Frame:UpdateLayout()
					Frame:UpdateScrollIndicator()

					if Frame.UpdateGameTooltip then
						Frame:UpdateGameTooltip()
					end
				end
				...
				for i = 1, NS.Variables.Num_Spell do
					...
					if garrFollowerID and not spellTexture then
						...
						if followerInfo then
							spellTexture = followerInfo.portraitIconID
						end
					end

					Buttons[i].garrFollowerID = garrFollowerID
					Buttons[i].rewardSpellID = spellID
					...
					do -- Icon
						...
						if garrFollowerID then
							...
							if followerInfo and followerInfo.portraitIconID then
								Buttons[i].Image.IconTexture:SetTexture(followerInfo.portraitIconID)
								iconSet = true
							end
						end

						if not iconSet and spellTexture then
							Buttons[i].Image.IconTexture:SetTexture(spellTexture)
							iconSet = true
						end

						if not iconSet and CALLBACK_ICON then
							if CALLBACK_ICON:GetAtlas() then
								Buttons[i].Image.IconTexture:SetAtlas(CALLBACK_ICON:GetAtlas())
								iconSet = true
							elseif CALLBACK_ICON:GetTexture() then
								Buttons[i].Image.IconTexture:SetTexture(CALLBACK_ICON:GetTexture())
								iconSet = true
							end
						end
