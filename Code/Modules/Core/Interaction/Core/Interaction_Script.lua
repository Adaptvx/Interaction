...
function NS.Script:Load()
	...
	do
		...
		function NS.Script:Stop(skip, deselectTarget)
			...
			NS.Variables.CurrentSession.unit = nil
		...
		function NS.Script:Start(frameType)
			local NPC = addon.API.Util:GetFirstValue(UnitName("npc"), UnitName("questnpc"))
			local NPCUnit = addon.API.Util:GetInteractionUnitToken()
			...
			if (NS.Variables.CurrentSession.unit == NPCUnit) and (NS.Variables.CurrentSession.type == frameType) and (NS.Variables.CurrentSession.questID == QuestID) and addon.API.Util:AreValuesEqual(NS.Variables.CurrentSession.dialogText, TEXT) then
				return
			end
			...
			NS.Variables.SessionCounter = NS.Variables.SessionCounter + 1
			NS.Variables.CurrentSession.id = NS.Variables.SessionCounter
			...
			NS.Variables.CurrentSession.unit = NPCUnit
	...
	do
		...
		function NS.Script:Manager_Gossip_Visibility()
			local isNewNPC = (NS.Variables.CurrentSession.id ~= addon.Interaction.Gossip.Variables.CurrentSession.id)
				...
				f:SetScript("OnEvent", function(self, event, arg1, arg2)
					if event == "PLAYER_REGEN_DISABLED" then
						if addon.HideUI.Variables.Active and not Combat_AlertShowForSession then
							...
							addon.Alert.Script:Show(addon.Variables.PATH_ART .. "Alert\\Swords.png", L["Alert - Under Attack"], 17.5, nil, nil)
						end
					end
				end)
				...
				CallbackRegistry:Add("START_INTERACTION", function()
					if not addon.Database.DB_GLOBAL.profile.TutorialSettingsShown then
						...
						if addon.Input.Variables.IsController then
							Shortcut = addon.API.Util:InlineIcon(addon.Variables.PATH_ART .. "Blizzard\\Settings\\Shortcut-GamePad.png", 25, 25, 0, 0) .. L["Alert - Open Settings"]
						else
							Shortcut = addon.API.Util:InlineIcon(addon.Variables.PATH_ART .. "Blizzard\\Settings\\Shortcut-KBM.png", 25, 75, 0, 0) .. L["Alert - Open Settings"]
						end
