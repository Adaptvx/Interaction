...
do
	do -- Button
		...
		function NS.Styles:Button(frame, data)
			...
			frame._Font = GameFontNormal:GetFont()
			...
			do -- Theme
				local function UpdateTheme()
					...
					if (theme and theme == 2) or (not theme and addon.API.Util.IsDarkTheme) then
						frame._DefaultTexture = frame._CustomDefaultTexture or addon.API.Presets.TEXTURE_INSCRIBED_BORDER
						frame._HighlightTexture = frame._CustomHighlightTexture or addon.API.Presets.TEXTURE_INSCRIBED
			...
			do -- Events
				local f = CreateFrame("Frame", "$parent.Events", frame)
