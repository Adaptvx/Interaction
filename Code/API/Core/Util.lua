...
do -- Tooltip helpers
	...
	function addon.API.Util:RemoveTooltip(frame)
		...
		frame.tooltipText = ""
		if frame.tooltipActive then frame.tooltipActive = false frame.API_HideTooltip() end
	end
end
...
do -- Watchers
	...
	function addon.API.Util:WatchLocalVariable(variableTable, variableName, callback)
		local mt = {
			__newindex = function(t, key, value) rawset(t, key, value) TriggerCallbacks(variableName, value) end,
...
do -- General helpers
	function addon.API.Util:tnum(tbl) local length = 0 for _ in pairs(tbl) do length = length + 1 end return length end
	function addon.API.Util:rt(tbl) local reversed, len = {}, #tbl for i = len, 1, -1 do reversed[len - i + 1] = tbl[i] end return reversed end
	function addon.API.Util:gen_hash() local hash, chars = "", "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789" for i = 1, 16 do local rand = math.random(1, #chars) hash = hash .. chars:sub(rand, rand) end return hash end
end
